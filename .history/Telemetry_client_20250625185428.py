...
class TelemetryClient:
    ...
    def parse_telemetry_data(self, arbitration_id, data): # data comes pre-received from the input loop
        ...
        try:
            ...
            logger.info("Received CAN message with ID=%s, Data=%s", arbitration_id, data) # too lazy to do this after the type, will do it before
            ...
            if arbitration_id == '0x1e': # BMS STM32 Board CAN ID, should not be a string when testing with CAN
                ...
                # print(f"telemetry:{arbitration_id}:{data}") # can print the full message id on this client, however needs to show up parsed/described on dashboard
                return f"Telemetry: BMS: {arbitration_id}:{data}" # don't need the arbitration id anymore, can still return it here to test it
