...
class MotorControlClient:
            ...
            if response.sender == "dashboard" and response.command.startswith("motor:"):
                try:
                    ...
                    if len(command_parts) >= 3:
                        ...
                        if len(data_bytes) > 0:
                            if data_bytes[0] == 1 and len(data_bytes) >= 2:  # MC_START
                                print(f"[Motor] Starting motor with {data_bytes[1]}% throttle")
                            elif data_bytes[0] == 2:  # MC_STOP
                                print(f"[Motor] Stopping motor")
                            elif data_bytes[0] == 3 and len(data_bytes) >= 2:  # MC_THROTTLE
                                print(f"[Motor] Setting throttle to {data_bytes[1]}%")
                            elif data_bytes[0] == 4:  # MC_DIRECTION
                                print(f"[Motor] Toggling motor direction")
                            elif data_bytes[0] == 0:  # MC_IDLE
                                print(f"[Motor] Setting motor to idle")
    ...
    #         if len(data) > 0:
    #             cmd_type = {0: "IDLE", 1: "START", 2: "STOP", 3: "THROTTLE", 4: "DIRECTION"}.get(data[0], "UNKNOWN")
...
# Motor Control Commands (MOTOR_CONTROLLER CAN ID = 0x123)
    ...
    if len(parts) == 2:
        try:
            ...
            if 0 <= throttle <= 100:
                command = f"motor:291:1,{throttle}"  # MC_START (0x01) + throttle value
            else:
    ...
    command = f"motor:291:2"  # MC_STOP (0x02)
    ...
    if len(parts) == 2:
        try:
            throttle = int(parts[1])
            if 0 <= throttle <= 100:
                command = f"motor:291:3,{throttle}"  # MC_THROTTLE (0x03) + throttle value
            else:
                print("[Dashboard] Throttle must be 0-100")
                continue
        except ValueError:
    ...
    command = f"motor:291:4"  # MC_DIRECTION (0x04)

elif user_input.lower() == 'idle':
    command = f"motor:291:0"  # MC_IDLE (0x00)