        ...
        try:
            ...
            if arbitration_id == '0x1e': # BMS STM32 Board CAN ID, should not be a string when testing with CAN
                ...
                if len(data_bytes) <= 8:
                    ...
                    MUX2_TEMP = f"0x{int(data_bytes[1]):02x}"
                    MUX3_TEMP = f"0x{int(data_bytes[2]):02x}"
                    MUX4_TEMP = f"0x{int(data_bytes[3]):02x}"
                    MUX5_TEMP = f"0x{int(data_bytes[4]):02x}"
                    MUX6_TEMP = f"0x{int(data_bytes[5]):02x}"
                    ERROR_ID = data_bytes[6]
                    return f"Telemetry: SENSORS: MUX1_TEMP={MUX1_TEMP}, MUX2_TEMP={MUX2_TEMP}, MUX3_TEMP={MUX3_TEMP}, MUX4_TEMP={MUX4_TEMP}, MUX5_TEMP={MUX5_TEMP}, MUX6_TEMP={MUX6_TEMP}, ERROR_ID={ERROR_ID}"
                else:
